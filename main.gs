...
checkUpdate = function(responder) {
  ...
  var response = UrlFetchApp.fetch("https://raw.githubusercontent.com/xicholo/miyamoto/master/VERSION", {muteHttpExceptions: true});

  if(response.getResponseCode() == 200) {
    var latest_version = parseFloat(response.getContentText());
    if(latest_version > 0 && latest_version > current_version) {
      responder.send("最新のみやもとさんの準備ができました！\nhttps://github.com/xicholo/miyamoto/blob/master/UPDATE.md を読んでください。");

      var response = UrlFetchApp.fetch("https://raw.githubusercontent.com/xicholo/miyamoto/master/HISTORY.md", {muteHttpExceptions: true});
      if(response.getResponseCode() == 200) {
...
function migrate() {
  ...
  var global_settings = new GASProperties();
  global_settings.set('version', "20180705.0");
